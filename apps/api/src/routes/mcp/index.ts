  ...
  const mcpServer = new McpServer(
    { name: "voiceci", version: "0.5.0" },
    {
      ...
      instructions: [
        ...
        "  - If NO → call voiceci_get_scenario_guide, read agent code, design tests, and create the `voiceci/` folder with both `audio.json` and `conversations.json`.",
        "",
        "Step 2: Read `voiceci/audio.json`, then call voiceci_run_suite with its contents as the `config` parameter. By default, only run audio tests.",
        "  - To also run conversation tests: read `voiceci/conversations.json`, merge it into the config (spread/Object.assign), then call voiceci_run_suite with the merged config.",
